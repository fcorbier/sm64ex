    ...
    if (node->header.gfx.unk18 == gCurGraphNodeRoot->areaIndex) {
        if (node->header.gfx.throwMatrix != NULL) {
            ...
            if (gGlobalTimer == node->header.gfx.prevThrowMatrixTimestamp + 1 &&
                ...
                interpolate_matrix(mtxf, *(Mat4*)node->header.gfx.throwMatrix, node->header.gfx.prevThrowMatrix);
            ...
            mtxf_copy(node->header.gfx.prevThrowMatrix, *(Mat4*)node->header.gfx.throwMatrix);
